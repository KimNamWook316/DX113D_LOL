    ...
    float3 Binormal, float2 UV, float2 OriginUV)
{
	...
	if (g_MtrlBumpEnable == 1)
	{
		...
		{
			float4 SplatNormal = g_SplatNormal.Sample(g_BaseSmp, float3(OriginUV, i));
...
PSOutput_GBuffer LandScapePS(Vertex3DOutput input)
{
	...
	for (int i = 0; i < g_LandScapeSplatCount; ++i)
	{
		float4 SplatDiffuse = g_SplatDiffuse.Sample(g_BaseSmp, float3(input.UV, i));
	...
	output.GBuffer1.rgb = ComputeBumpNormal(input.Normal, input.Tangent, input.Binormal, UV);
	...
	for (int i = 0; i < g_LandScapeSplatCount; ++i)
	{
		float4 SplatSpecular = g_SplatSpecular.Sample(g_BaseSmp, float3(input.UV, i));
